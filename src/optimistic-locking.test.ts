...
function delegateWith(
  updateCount: number,
  probeResult: { version: number } | null,
): VersionedDelegate<Record<string, unknown>, Record<string, unknown>> & {
...
describe("updateWithVersionGuard (Track G.2)", () => {
  it("matches on (id, tenantId, version) and increments atomically", async () => {
    ...
    const affected = await updateWithVersionGuard(delegate, target, {
      status: "CONFIRMED",
    });
  ...
  it("throws StaleWriteError with the current version when the row moved on", async () => {
    ...
    await expect(
      updateWithVersionGuard(delegate, target, { status: "X" }),
  ...
  it("throws RecordNotFoundForUpdateError when no row exists for the tenant", async () => {
    ...
    await expect(updateWithVersionGuard(delegate, target, {})).rejects.toThrow(
      RecordNotFoundForUpdateError,
    );
  });
  ...
  it("rejects invalid expectedVersion and reserved data fields", async () => {
    const delegate = delegateWith(1, null);
    await expect(
      updateWithVersionGuard(delegate, { ...target, expectedVersion: 0 }, {}),
    ).rejects.toThrow(RangeError);
    await expect(
      updateWithVersionGuard(delegate, { ...target, expectedVersion: 1.5 }, {}),
    ).rejects.toThrow(RangeError);
    await expect(
      updateWithVersionGuard(delegate, target, { version: 9 }),
    ).rejects.toThrow(RangeError);
    await expect(
      updateWithVersionGuard(delegate, target, { tenantId: "evil" }),
    ).rejects.toThrow(RangeError);
