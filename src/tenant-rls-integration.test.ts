...
let tenantRecordA: { id: string } | null = null;
let tenantRecordB: { id: string } | null = null;

beforeAll(async () => {
  // Clean up any previous test data (order matters for FK constraints)
  await prisma.$executeRawUnsafe(`DELETE FROM "customers" WHERE "tenant_id" IN ($1, $2)`, TENANT_A, TENANT_B);
  await prisma.$executeRawUnsafe(`DELETE FROM "organizations" WHERE "tenant_id" IN ($1, $2)`, TENANT_A, TENANT_B);
  await prisma.$executeRawUnsafe(`DELETE FROM "users" WHERE "tenant_id" IN ($1, $2)`, TENANT_A, TENANT_B);
  await prisma.$executeRawUnsafe(`DELETE FROM "tenants" WHERE "id" IN ($1, $2)`, TENANT_A, TENANT_B);

  // Create Tenant records matching our test constants so FK constraints are satisfied
  const tenantA = await prisma.tenant.upsert({
    where: { id: TENANT_A },
    create: { id: TENANT_A, name: 'RLS Test Tenant A', slug: 'rls-test-tenant-a' },
    update: {},
  });
  tenantRecordA = tenantA;
  const tenantB = await prisma.tenant.upsert({
    where: { id: TENANT_B },
    create: { id: TENANT_B, name: 'RLS Test Tenant B', slug: 'rls-test-tenant-b' },
    update: {},
  });
  tenantRecordB = tenantB;
});

describe('RLS: Role and policy baseline', () => {
  it('runs under a non-bypass role (unerp_api expected)', async () => {
    const [row] = await prisma.$queryRawUnsafe<Array<{ rolsuper: boolean; rolbypassrls: boolean }>>(
      `SELECT rolsuper, rolbypassrls FROM pg_roles WHERE rolname = current_user`
    );
    expect(row).toBeDefined();
    if (row.rolsuper) {
      console.warn('WARN: connected as superuser — RLS is bypassed; isolation tests below should fail');
    }
  });
  ...
  it('every tenant-scoped table has a tenant_isolation policy', async () => {
    const missingPolicies = await prisma.$queryRawUnsafe<Array<{ tablename: string }>>(
...
describe('RLS: Two-tenant data isolation', () => {
  it('creates test data for tenant A and tenant B under tenant sessions', async () => {
    await runWithTenantSession({ tenantId: TENANT_A, userId: USER_A }, async () => {
      const org = await prisma.organization.create({
        data: { name: 'Org A' },
      });
      testOrgIdA = org.id;
      const c = await prisma.customer.create({
        data: { name: 'Tenant A Customer', email: 'a@test.com', status: 'ACTIVE', orgId: org.id },
      });
      testCustomerIdA = c.id;
    });

    await runWithTenantSession({ tenantId: TENANT_B, userId: USER_B }, async () => {
      const org = await prisma.organization.create({
        data: { name: 'Org B' },
      });
      testOrgIdB = org.id;
      const c = await prisma.customer.create({
        data: { name: 'Tenant B Customer', email: 'b@test.com', status: 'ACTIVE', orgId: org.id },
      });
  ...
  it('tenant A cannot see any rows for tenant B', async () => {
    await runWithTenantSession({ tenantId: TENANT_A, userId: USER_A }, async () => {
      const customers = await prisma.customer.findMany({ where: { email: 'b@test.com' } });
  ...
  it('tenant B cannot see any rows for tenant A', async () => {
    await runWithTenantSession({ tenantId: TENANT_B, userId: USER_B }, async () => {
      const customers = await prisma.customer.findMany({ where: { email: 'a@test.com' } });
      expect(customers).toHaveLength(0);
    });
  });
  ...
  it('tenant A count excludes tenant B rows', async () => {
    await runWithTenantSession({ tenantId: TENANT_A, userId: USER_A }, async () => {
      const count = await prisma.customer.count({ where: { name: { contains: 'Tenant' } } });
      expect(count).toBe(1);
    });
  });
  ...
  it('tenant A cannot read tenant B row by direct ID', async () => {
    await runWithTenantSession({ tenantId: TENANT_A, userId: USER_A }, async () => {
      const customer = await prisma.customer.findUnique({ where: { id: testCustomerIdB } });
  ...
  it('tenant A cannot read tenant B row by raw SQL', async () => {
    ...
    await runWithTenantSession({ tenantId: TENANT_A, userId: USER_A }, async () => {
      ...
      if (role?.rolsuper) {
        console.warn('SKIP (superuser): raw SQL isolation not enforceable on this connection');
        return;
      }
...
describe('RLS: No-context returns no rows', () => {
  it('returns no rows when querying outside a tenant session', async () => {
    ...
    if (role?.rolsuper) {
      console.warn('SKIP: connected as superuser — RLS bypassed, no-context count is unrestricted');
      return;
    }
  ...
  it('session tenant_id overrides caller-supplied value', async () => {
    await runWithTenantSession({ tenantId: TENANT_A, userId: USER_A }, async () => {
      const customer = await prisma.customer.create({
        data: { name: 'Spoof Attempt', email: 'spoof@test.com', status: 'ACTIVE', orgId: testOrgIdA },
      });
      ...
      await runWithTenantSession({ tenantId: TENANT_B, userId: USER_B }, async () => {
        const notFound = await prisma.customer.findUnique({ where: { id: customer.id } });
